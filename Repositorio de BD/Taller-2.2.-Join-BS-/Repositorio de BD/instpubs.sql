...
-- Consulta 1 Autores y sus libros
SELECT a.au_fname, a.au_lname, t.title
FROM authors a
INNER JOIN titleauthor ta ON a.au_id = ta.au_id
INNER JOIN titles t ON ta.title_id = t.title_id;

--Consulta 2 Libros y editoriales
SELECT t.title, p.pub_name
FROM titles t
INNER JOIN publishers p ON t.pub_id = p.pub_id;

-- Consulta 3 Ventas y libros
SELECT s.qty, t.title
FROM sales s
INNER JOIN titles t ON s.title_id = t.title_id;

--Consulta 4 Tiendas y ventas
SELECT st.stor_name, s.qty
FROM stores st
INNER JOIN sales s ON st.stor_id = s.stor_id;

--Consulta 5 Autores y regalías
SELECT a.au_fname, a.au_lname, ta.royaltyper
FROM authors a
INNER JOIN titleauthor ta ON a.au_id = ta.au_id;

--Consulta 6 Libros con sus autores
SELECT t.title, a.au_fname, a.au_lname
FROM titles t
INNER JOIN titleauthor ta ON t.title_id = ta.title_id
INNER JOIN authors a ON ta.au_id = a.au_id;

--Consulta 7 Libros vendidos en cada tienda
SELECT st.stor_name, t.title
FROM stores st
INNER JOIN sales s ON st.stor_id = s.stor_id
INNER JOIN titles t ON s.title_id = t.title_id;

--Consulta 8 Libros, precio y cantidad vendida
SELECT t.title, t.price, s.qty
FROM titles t
INNER JOIN sales s ON t.title_id = s.title_id;

--Consulta 9 Autores que no tienen libros
SELECT a.au_fname, a.au_lname
FROM authors a
LEFT JOIN titleauthor ta ON a.au_id = ta.au_id
WHERE ta.title_id IS NULL;

--Consulta 10 Libros que no tienen autores
SELECT t.title
FROM authors a
